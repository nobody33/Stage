...
#date de création : 13/05/19 par : Quentin Dhersin
#dernière modification le : 25/05/19 par : Quentin Dhersin

#cette version comporte toutes les fonctionnalités demandées en utilisant la version graphique V2
...
def recherche():
    ...
    for i in treeview.get_children():
        treeview.delete(i)
    #on crée la requête
    if champ_compte_window.get()=="":
        champ_compte_window.insert(0, "%")
    if champ_nom.get()=="":
        champ_nom.insert(0, "%")
    ...
    if liste_statut_compte['selection']=="Tous" or liste_statut_compte['selection']=="":
        requete = "SELECT * FROM user_skcdp WHERE CPTWIN LIKE \'" + champ_compte_window.get().upper() + "\' AND NOM LIKE \'" + champ_nom.get().upper() + "\' AND DIRECTION LIKE \'" + liste_direction['selection'] + "\' AND SERVICE LIKE \'" + liste_service['selection'] + "\' AND RHIERAR LIKE \'" + liste_responsable_hierarchique['selection'] + "\'"
    elif liste_statut_compte['selection']=="Ouvert":
        requete = "SELECT * FROM user_skcdp WHERE CPTWIN LIKE \'" + champ_compte_window.get().upper() + "\' AND NOM LIKE \'" + champ_nom.get().upper() + "\' AND DIRECTION LIKE \'" + liste_direction['selection'] + "\' AND SERVICE LIKE \'" + liste_service['selection'] + "\' AND RHIERAR LIKE \'" + liste_responsable_hierarchique['selection'] + "\' AND STATUT_OUV LIKE \'T\' AND (STATUT_FERM LIKE \'% %\' OR STATUT_FERM IS NULL)"
    elif liste_statut_compte['selection']=="Fermé":
        requete = "SELECT * FROM user_skcdp WHERE CPTWIN LIKE \'" + champ_compte_window.get().upper() + "\' AND NOM LIKE \'" + champ_nom.get().upper() + "\' AND DIRECTION LIKE \'" + liste_direction['selection'] + "\' AND SERVICE LIKE \'" + liste_service['selection'] + "\' AND RHIERAR LIKE \'" + liste_responsable_hierarchique['selection'] + "\' AND STATUT_OUV LIKE \'T\' AND STATUT_FERM LIKE \'T\'"
    #on lance la requête
    ...
    for row in cursor:
        ...
        date_fermeture_recherche=row[10]
        if len(date_fermeture_recherche) == 8:
            ...
            date_fermeture_recherche=jour_ouverture_recherche + '/' + mois_ouverture_recherche + '/' + annee_ouverture_recherche
    ...
    champ_compte_window.delete(0, "end")
...
def modifier():
    ...
    def modifierBD():
        ...
        if champ_compte_window_modifier.get() != "" and champ_nom_modifier.get() != "":
            ...
            if (len(champ_ouverture_date_modifier.get()) == 10 or len(champ_ouverture_date_modifier.get()) == 0) and (len(champ_fermeture_date_modifier.get()) == 10 or len(champ_fermeture_date_modifier.get()) == 0):
                ...
                treeview.insert("", 0, text=champ_compte_window_modifier.get().upper(), values=(liste_type_modifier['selection'], champ_nom_modifier.get().upper(),
            ...
            erreur = Label(fenetre_erreur_champs_vide, text = "Les champs Cpte windows et Nom doivent être remplis", bg="grey", fg="blue", font=("liberation serif", 12))
            erreur.pack()
    ...
    texte_sect_xng_modifier = Label(canvas_modifier, text="Sect Xng", bg="grey", font=("liberation serif", 12))
    ...
    texte_sect_xng_modifier.grid(row=6, column=2)
    ...
    champ_sect_xng_modifier.grid(row=7, column=2, padx=5)
...
def inserer():
    ...
    def ajouterBD():
        ...
        if champ_compte_window_inserer.get() != "" and champ_nom_inserer.get() != "":
        #on vérifie que la date est au bon format
            if (len(champ_ouverture_date_inserer.get()) == 10 or len(champ_ouverture_date_inserer.get()) == 0) and (len(champ_fermeture_date_inserer.get()) == 10 or len(champ_fermeture_date_inserer.get()) == 0):
                ...
                #on ajoute la nouvelle ligne dans le tableau
                treeview.insert("", 0, text=champ_compte_window_inserer.get().upper(), values=(liste_type_inserer['selection'], champ_nom_inserer.get().upper(),
            ...
            fenetre_erreur_champs_vide = tix.Toplevel()
            fenetre_erreur_champs_vide.title("Erreur")
            texte_erreur = Label(fenetre_erreur_champs_vide, text = "Les champs Cpte windows et Nom doivent être remplis", bg="grey", fg="blue", font=("liberation serif", 12))
            texte_erreur.pack()
            bouton_ok = Button(fenetre_erreur_champs_vide, text="OK", command=lambda:fenetre_erreur_champs_vide.destroy(), bg="grey")
            bouton_ok.pack()
            fenetre_erreur_champs_vide.configure(background="grey")
    ...
    texte_sect_xng_inserer = Label(canvas_insertion, text="Sect Xng", bg="grey", font=("liberation serif", 12))
    ...
    texte_sect_xng_inserer.grid(row=6, column=2)
    ...
    champ_sect_xng_inserer.grid(row=7, column=2, padx=5)
...
try:
    ...
    connexion_BD = cx_Oracle.connect('id/password@server')
...
canvas_entete = Canvas(fenetre_principale, width=1250, height=5, bg="grey", highlightthickness=0)

#on crée les textes à afficher en entête
texte_userskcp = Label(canvas_entete, text="USERSKCP", bg="grey", font=("liberation serif", 12))
texte_smurfit = Label(canvas_entete, text="SMURFIT-Cellulose du Pin", bg="grey", font=("liberation serif", 12))
texte_date_heure = Label(canvas_entete, text=date_complete, bg="grey", font=("liberation serif", 12))
texte_titre = Label(canvas_entete, text="Gestion des utilisateurs", bg="grey", font=("liberation serif", 12))
texte_informatique = Label(canvas_entete, text="INFORMATIQUE", bg="grey", font=("liberation serif", 12))

#on crée un canvas pour séparé l'entête du reste de la fenêtre
canvas_ligne = Canvas(fenetre_principale, width=100, height=5, bg="grey")

#on crée un canvas pour placer les champs à completer et leur titres
canvas_recherche = Canvas(fenetre_principale, width=1250, height=5, bg="grey", highlightthickness=0)

#on crée les textes à afficher au dessus des champs à compléter
texte_compte_window = Label(canvas_recherche, text="Compte Window", bg="grey", font=("liberation serif", 12))
texte_nom = Label(canvas_recherche, text="Nom", bg="grey", font=("liberation serif", 12))
texte_direction = Label(canvas_recherche, text="Direction", bg="grey", font=("liberation serif", 12))
texte_service = Label(canvas_recherche, text="Service", bg="grey", font=("liberation serif", 12))
texte_responsable_hierarchique = Label(canvas_recherche, text="Responsable hiérarchique", bg="grey", font=("liberation serif", 12))
texte_statut_compte = Label(canvas_recherche, text="Statut du compte", bg="grey", font=("liberation serif", 12))

#on crée les champs à compléter pour la recherche
champ_compte_window = Entry(canvas_recherche, width=20)
...
canvas_centre = Canvas(fenetre_principale, width=1200, height=450, bg="grey", highlightthickness=0)

#on crée un canvas pour le treeview
canvas_tableau = Canvas(canvas_centre, width=100, height=450, bg="grey", highlightthickness=0)
...
canvas_bouton = Canvas(fenetre_principale, width=100, height=100, bg="grey", highlightthickness=0)

#on crée les boutons
bouton_chercher = Button(canvas_recherche, text="Chercher", command=lambda:recherche(), font=("liberation serif", 12))
bouton_inserer = Button(canvas_bouton, text="Insérer", command=lambda:inserer(), font=("liberation serif", 12))
bouton_modifier = Button(canvas_bouton, text="Modifier", command=lambda:modifier(), font=("liberation serif", 12))
bouton_quitter = Button(canvas_bouton, text="Quitter", command=lambda:quitter(), font=("liberation serif", 12))
...
canvas_entete.pack(pady=10)

#placement des textes dans l'entête de l'application
texte_userskcp.grid(row=1, column=1, padx=10, pady=5)
texte_smurfit.grid(row=1, column=2, padx=300, pady=5)
texte_date_heure.grid(row=1, column=3, padx=10, pady=5)
texte_titre.grid(row=2, column=1, padx=10, pady=5)
texte_informatique.grid(row=2, column=3, padx=10, pady=5)

#placement du canvas_ligne de façon à ce qu'il occupe toute la largeur de la fenêtre
canvas_ligne.pack(fill=X, pady=10)
...
canvas_recherche.pack(pady=10)

#placement des textes au dessus des champs à remplir
texte_compte_window.grid(row=3, column=1, padx=20, pady=5)
...
champ_compte_window.grid(row=4, column=1, padx=20, pady=5)
